...
import { GET_TODOS_CONNECTION } from "./graphql/queries";
import { CREATE_TODO, UPDATE_TODO, DELETE_TODO } from "./graphql/mutations";
import { TodoItem } from "./components/TodoItem";
import { TodoForm } from "./components/TodoForm";
import { TodoConnection } from "./types";

function App() {
	const { data, loading, error, fetchMore } = useQuery<{
		todosConnection: TodoConnection;
	}>(GET_TODOS_CONNECTION, {
		variables: { first: 5 },
	});
	const [createTodo] = useMutation(CREATE_TODO, {
		refetchQueries: [{ query: GET_TODOS_CONNECTION, variables: { first: 5 } }],
	});
